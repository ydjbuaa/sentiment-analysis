...
def create_shared(out_size, in_size=None, name=None):
    ...
    if in_size is None:
        ...
        return theano.shared(random_initialization((in_size, out_size)), name=name)
...
class LogisticRegression2(object):
    ...
    """
    def __init__(self, x1, x2, input_size, output_size, prefix="lr2"):

        self.name = prefix
        self.w = create_shared(output_size, input_size, prefix+"_w")
        self.u = create_shared(output_size, input_size, prefix+"_u")
        self.b = create_shared(output_size, None, prefix+"_b")
        self.p_y_given_x = tensor.nnet.softmax(tensor.dot(x1, self.w)+tensor.dot(x2, self.u) + self.b)
